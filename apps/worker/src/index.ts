...
const REDIS_STREAM = "video_transcoding_jobs";
...
class VideoJobWorker {
  ...
  async handleJob(jobId: string, jobData: Record<string, string>) {
    try {
      ...
      console.error(`Error processing job ${jobData.videoId}:`, error);
    }
  }
